...
from keras.src.layers import LSTM, Dropout, Dense, Bidirectional
...
# 2. Build the BLSTM model
def build_BLSTM_model(Xtrain,Y_train,Xval,Y_val):
    model = Sequential()
    model.add(Bidirectional(LSTM(units=50, return_sequences=True), input_shape=(Xtrain.shape[1], Xtrain.shape[2])))
...
for i in tqdm(range(box-1)):
    ...
    for c in ['Open','High','Low','Close']:
        ...
        with tf.device(device):
            model=build_BLSTM_model(Xtrain,  Y_train["y_"+c],Xval, Y_val["y_"+c])
...
for c in ['Open','High','Low','Close']:
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f"BLSTM_{c}_{name}.png")
    plt.close()
    plt.cla()
